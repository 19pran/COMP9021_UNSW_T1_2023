...
if values[0] == values[-1]:
    ...
    print(f"The first value is {'greater' if values[0] > values[-1] else 'smaller'} than the last value.")
...
def draw_long_horizontal_line():
    return f"   {('-' * (the_max * 3 + 2))}"


def draw_short_horizontal_line():
    return f"   {('-' * (the_max * 2 + 1))}"
...
long_lines = [
    f"   | {'*  ' * e}{'   ' * (the_max - e)}|"
    if e != the_max else f"   | {'*  ' * (e - 1)}* |"
    for e in values
]

output_long_lines = '\n'.join([draw_long_horizontal_line()] + long_lines + [draw_long_horizontal_line()])

print(output_long_lines)
...
short_lines = [
    f"   |{'*|'*e if e==the_max else ' |'*(the_max-e)+'*|'*e}"
    for e in values
]

output_short_lines = '\n'.join([draw_short_horizontal_line()] + short_lines + [draw_short_horizontal_line()])

print(output_short_lines)
